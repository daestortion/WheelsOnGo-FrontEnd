...
export const Login = () => {
  ...
  useEffect(() => {
    const clearLocalStorageIfEmpty = async () => {
      try {
        const response = await axios.get("https://tender-curiosity-production.up.railway.app/user/checkDatabaseEmpty");
        if (response.data) {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      const response = await axios.post("https://tender-curiosity-production.up.railway.app/user/login", {
        identifier,
        password,
      });
      ...
      if (response.data && response.data.userId) {
        ...
        try {
          const userProfileResponse = await axios.get(`https://tender-curiosity-production.up.railway.app/user/getUserById/${userId}`);
          const userProfile = userProfileResponse.data;
  
          // Fetch verification status
          try {
            const verificationResponse = await axios.get(`https://tender-curiosity-production.up.railway.app/verification/getVerificationByUserId/${userId}`);
